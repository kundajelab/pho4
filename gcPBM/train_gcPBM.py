...
inputs_coordstovals = coordstovals.core.CoordsToValsJoiner(
    coordstovals_list=[
      ...
      coordstovals.bigwig.PosAndNegSmoothWindowCollapsedLogCounts(
        pos_strand_bigwig_path="/users/amr1/pho4/data/gcpbm/"+target+"/ctl/control_pos_strand.bw",
        neg_strand_bigwig_path="/users/amr1/pho4/data/gcpbm/"+target+"/ctl/control_neg_strand.bw",
...
targets_coordstovals = coordstovals.bigwig.PosAndNegSeparateLogCounts(
    pos_strand_bigwig_path="/users/amr1/pho4/data/gcpbm/"+target+"/basename_prefix.pooled.positive.bigwig",
    neg_strand_bigwig_path="/users/amr1/pho4/data/gcpbm/"+target+"/basename_prefix.pooled.negative.bigwig",
...
keras_train_batch_generator = coordbased.core.KerasBatchGenerator(
  coordsbatch_producer=coordbatchproducers.SimpleCoordsBatchProducer(
      bed_file="/users/amr1/pho4/data/gcpbm/"+target+"/train_1k_around_summits.bed.gz",
      coord_batch_transformer=
          coordbatchtransformers.UniformJitter(
              maxshift=200, chromsizes_file="/users/amr1/pho4/data/genome/hg38/hg38.chrom.sizes"),
      batch_size=128,
...
keras_valid_batch_generator = coordbased.core.KerasBatchGenerator(
  coordsbatch_producer=coordbatchproducers.SimpleCoordsBatchProducer(
            bed_file="/users/amr1/pho4/data/gcpbm/"+target+"/valid_1k_around_summits.bed.gz",
            batch_size=128,
...
model.save('/users/amr1/pho4/data/models/'+target+'_model.h5')

keras_test_batch_generator = coordbased.core.KerasBatchGenerator(
  coordsbatch_producer=coordbatchproducers.SimpleCoordsBatchProducer(
            bed_file="/users/amr1/pho4/data/gcpbm/"+target+"/test_1k_around_summits.bed.gz",
            batch_size=128,
            shuffle_before_epoch=False, 
            seed=1234),
  inputs_coordstovals=inputs_coordstovals,
...
plt.title(str(spearmanr(test_biastrack_logcount, test_labels_logtotalcount)[0])+", "+
          str(pearsonr(test_biastrack_logcount, test_labels_logtotalcount)[0]))
plt.plot([np.min(test_biastrack_logcount), np.max(test_biastrack_logcount)],
         ...
         color="black")
plt.savefig('figs/'+target+'_test_bias.png')
plt.clf()
...
plt.title(str(spearmanr(test_preds_logcount[:,0], test_labels_logcount[:,0])[0])+", "+
          str(pearsonr(test_preds_logcount[:,0], test_labels_logcount[:,0])[0]))
plt.plot([np.min(test_preds_logcount[:,0]), np.max(test_preds_logcount[:,0])],
         ...
         color="black")
plt.savefig('figs/'+target+'_test_fwd.png')
plt.clf()
...
plt.title(str(spearmanr(test_preds_logcount[:,1], test_labels_logcount[:,1])[0])+", "+
          str(pearsonr(test_preds_logcount[:,1], test_labels_logcount[:,1])[0]))
plt.plot([np.min(test_preds_logcount[:,1]), np.max(test_preds_logcount[:,1])],
         ...
         color="black")
plt.savefig('figs/'+target+'_test_rev.png')
plt.clf()
...
plt.xlabel("Bias track log counts")
plt.ylabel("True log total counts")
plt.title(str(spearmanr(o_train_biastrack_logcount, o_train_labels_logtotalcount)[0])+", "+
          str(pearsonr(o_train_biastrack_logcount, o_train_labels_logtotalcount)[0]))
plt.plot([np.min(o_train_biastrack_logcount), np.max(o_train_biastrack_logcount)],
         ...
         color="black")
plt.savefig('figs/'+target+'_train_bias.png')
plt.clf()
...
plt.xlabel("Predicted log counts - Forward Strand")
plt.ylabel("True log counts - Forward Strand")
plt.title(str(spearmanr(o_train_preds_logcount[:,0], o_train_labels_logcount[:,0])[0])+", "+
          str(pearsonr(o_train_preds_logcount[:,0], o_train_labels_logcount[:,0])[0]))
plt.plot([np.min(o_train_preds_logcount[:,0]), np.max(o_train_preds_logcount[:,0])],
         ...
         color="black")
plt.savefig('figs/'+target+'_train_fwd.png')
plt.clf()

plt.scatter(o_train_preds_logcount[:,1], o_train_labels_logcount[:,1], alpha=0.1)
plt.xlabel("Predicted log counts - Reverse Strand")
plt.ylabel("True log counts - Reverse Strand")
plt.title(str(spearmanr(o_train_preds_logcount[:,1], o_train_labels_logcount[:,1])[0])+", "+
          str(pearsonr(o_train_preds_logcount[:,1], o_train_labels_logcount[:,1])[0]))
plt.plot([np.min(o_train_preds_logcount[:,1]), np.max(o_train_preds_logcount[:,1])],
         ...
         color="black")
plt.savefig('figs/'+target+'_train_rev.png')
plt.clf()
...
for idx in sorted_test_indices[:5]:
    ...
    for oneovertemp in [1.0]:
        ...
        plt.savefig('figs/profiles/'+target+str(idx)+'.png')
        plt.clf()