...
for target in targets:
    all_post_counts_hypimps = np.load('imp-scores/'+target+'/post_counts_hypimps.npy')
    all_post_profile_hypimps = np.load('imp-scores/'+target+'/post_profile_hypimps.npy') 
    all_post_counts_actualimps = np.load('imp-scores/'+target+'/post_counts_actualimps.npy') 
    all_post_profile_actualimps = np.load('imp-scores/'+target+'/post_profile_actualimps.npy') 
    all_labels_profile = np.load('imp-scores/'+target+'/labels_profile.npy') 
    all_labels_logcount = np.load('imp-scores/'+target+'/labels_logcount.npy') 
    all_preds_profile = np.load('imp-scores/'+target+'/preds_profile.npy') 
    all_preds_logcount = np.load('imp-scores/'+target+'/preds_logcount.npy') 
    all_biastrack_profile = np.load('imp-scores/'+target+'/biastrack_profile.npy') 
    all_biastrack_logcount = np.load('imp-scores/'+target+'/biastrack_logcount.npy') 
    all_seqs = np.load('imp-scores/'+target+'/seqs.npy') 
    ...
    if not os.path.exists('modisco/'+target):
        os.makedirs('modisco/'+target)
    ...
    counts_tfmodisco_results = modisco.tfmodisco_workflow.workflow.TfModiscoWorkflow(
            ...
            one_hot=all_seqs)
    counts_grp = h5py.File('modisco/'+target+"/counts_results.hdf5")
    ...
    profile_tfmodisco_results = modisco.tfmodisco_workflow.workflow.TfModiscoWorkflow(
            ...
            one_hot=all_seqs)
    profile_grp = h5py.File('modisco/'+target+"/profile_results.hdf5")
