...
repeatLen = 30
moveUntil = int((seq_len-len(motif))/2)-repeatLen
start = int((seq_len/2)-(len(motif)/2))
end = int((seq_len/2)+(len(motif)/2))

# left only
repeat = getRepeat(repeatLen, options.repeat, stack='left', mirror=False)
seqs = [background[:dist]+repeat+ \
        background[dist+repeatLen:start]+ \
        motif+background[end:] for dist in range(moveUntil)]  
predictPlotAndSave(seqs, "left")

# right only
repeat = getRepeat(repeatLen, repeatPattern, stack='right', mirror=False)
seqs = [background[:start]+motif+ \
        background[end:seq_len-repeatLen-dist]+ \
        repeat+background[seq_len-dist:] for dist in range(moveUntil)]  
predictPlotAndSave(seqs, "right")

# both
leftRepeat, rightRepeat = getRepeat(repeatLen, repeatPattern, stack='both', mirror=False)
seqs = [background[:dist]+leftRepeat+ \
        background[dist+repeatLen:start]+motif+ \
        background[end:seq_len-repeatLen-dist]+ \
        rightRepeat+background[seq_len-dist:] for dist in range(moveUntil)] 
