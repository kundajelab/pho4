...
parser.add_option('--flanks_txt',
    action="store", dest="flanks_txt",
...
flanks = []
firstLine = True
with open(options.flanks_txt) as inp:
    for line in inp:
        if firstLine:
            firstLine = False
            continue
        flanks.append(line.strip().split('\t')[1])
...
PBM_original="NNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNGTCTTGATTCGCTTGACGCTGCTG"
...
for seq in seq_peaks:
    ...
    candidates.append(fill_into_center(seq, PBM_original))
...
for flank_id, flank in enumerate(flanks):
    seqs.append(fill_into_center(background, "N"*24+flank))
    if flank_id % 100 == 0: 
        if preds.shape[0] == 0:
...
with open(options.output_json+"_PBM_fixed_min.json", 'w') as fp:
